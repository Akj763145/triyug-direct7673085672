import { Bell, Search, User, Menu, PanelLeftClose, PanelLeftOpen } from "lucide-react";
  ...
  return (
    <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-6 sticky top-0 z-10 w-full">
      <div className="flex-1 flex justify-start items-center gap-4">
        ...
        <div className="relative w-full max-w-sm hidden sm:block mx-auto flex-1">
          <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
          <Input 
            type="search"
            placeholder="Search by ID or name (Press /)"
            className="w-full bg-slate-100/50 pl-9 border-transparent text-sm focus-visible:ring-primary focus-visible:bg-white transition-all rounded-full"
          />
        </div>
      </div>
      <div className="flex items-center space-x-3 ml-auto">
        <Button variant="outline" size="icon" className="text-slate-500 border-slate-200 bg-white hover:bg-slate-50 hover:text-slate-800 relative rounded-full">
